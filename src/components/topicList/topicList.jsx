...
const topicList = () => {
  ...
  const requestList = () => {
    setTimeout(async () => {
      let url =
        "/api/topic?lastCursor=" + list[list.length - 1].order + "&pageSize=10";
      axios
        .get(url)
        .then((res) => {
          let newlist = [...res.data.data];
  ...
  return (
    <div className="left2">
      <InfiniteScroll
        ...
        next={requestList}
        hasMore={true}
        endMessage={<h2 className="load">End</h2>}
        loader={
          ...
          >
            <div className="head">
              <div className="title">
                ...
                <span className="time">
                  {calculate(dayjs(item.createdAt).unix())}
                </span>
              </div>
            </div>
            <div className={item.isShow === true ? "content" : "unfoldContent"}>
              {item.summary}
            </div>
            <ul className={item.isShow === true ? "news" : "newsShow"}>
              {item.newsArray.map((item1) => (
                <li key={item1.id} className="newsArr">
                  <a href={item1.url} target="_blank" className="new">
                    {item1.title}
                  </a>
                  <div className="from">{item1.siteName}</div>
                </li>
              ))}
            </ul>
